...
func loadProvinceConfigs() ([]ProvinceConfig, error) {
	...
	data, err := fileUtil.ReadFile("province_configs.json")
	if err != nil {
...
func saveProvinceResult(province string, result CrawlResult) error {
	...
	fileUtil := scripts.NewFileWriter()
	return fileUtil.SaveToFile(data, filename)
}
...
func saveAllUniversities(universities []UniversityInfo) error {
	...
	fileUtil := scripts.NewFileWriter()
	return fileUtil.SaveToFile(data, "all_universities.json")
}
...
func saveCrawlReport(results []CrawlResult) error {
	...
	fileUtil := scripts.NewFileWriter()
	return fileUtil.SaveToFile(data, "crawl_report.json")
}
