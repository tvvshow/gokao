...
func TestDebugRoute(t *testing.T) {
	...
	r.ServeHTTP(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("/debug/routes expected 200, got %d", w.Code)
	}
}

// TestSimpleRoute verifies that a simple route is working
func TestSimpleRoute(t *testing.T) {
	...
	// Test simple route
	w := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/test/proxy", nil)
	r.ServeHTTP(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("/test/proxy expected 200, got %d", w.Code)
	}
}

// TestProxyRoutes verifies that proxy routes are correctly set up
func TestProxyRoutes(t *testing.T) {
	gin.SetMode(gin.TestMode)
	r := setupRouter()

	// Test simple proxy route
	w := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/test/proxy", nil)
	r.ServeHTTP(w, req)
	
	// We expect the proxy logic to be called, which means we should not get 404
	// If we get 404, it means the route is not registered
	if w.Code == http.StatusNotFound {
		t.Fatalf("/test/proxy expected not 404, got %d", w.Code)
	}
