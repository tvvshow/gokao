...
// DeviceLicense 设备许可证模型
type DeviceLicense struct {
	ID           uuid.UUID      `gorm:"type:uuid;primary_key;default:gen_random_uuid()" json:"id"`
	UserID       uuid.UUID      `gorm:"type:uuid;not null;index" json:"user_id"`
	DeviceID     string         `gorm:"not null;size:255;index" json:"device_id"`
	LicenseData  string         `gorm:"type:text;not null" json:"license_data"`
	Status       string         `gorm:"not null;size:20;index" json:"status" validate:"required,oneof=active expired revoked"`
