...
import type { StudentInfo, Recommendation, RecommendationScheme } from '@/types/recommendation'
...
export const useRecommendationStore = defineStore('recommendation', () => {
  ...
  const generateRecommendations = async (studentData: StudentInfo) => {
    ...
    try {
      const response = await recommendationApi.generateRecommendations(studentData)
  ...
  // 保存推荐方案
  const saveScheme = async (schemeData: { name: string; studentInfo: StudentInfo; recommendations: Recommendation[] }) => {
    try {
  ...
  const loadSavedSchemes = async () => {
    try {
      const response = await recommendationApi.getSavedSchemes()
      
      if (response.success) {
        savedSchemes.value = response.data.schemes
  ...
  const deleteScheme = async (schemeId: string) => {
    try {
      const response = await recommendationApi.deleteScheme(schemeId)
      
      if (response.success) {
        savedSchemes.value = savedSchemes.value.filter(scheme => scheme.id !== schemeId)
      } else {
