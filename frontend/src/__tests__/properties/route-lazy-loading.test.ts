...
function analyzeRouteConfig(filePath: string): {
  ...
  while ((match = pathRegex.exec(content)) !== null) {
    ...
    const componentMatch = afterPath.match(/component\s*:\s*(\(\)\s*=>\s*import|[\w]+)/)
    
    const isLazy = componentMatch ? componentMatch[1].includes('import') : false
...
describe('Property 8: Route Lazy Loading', () => {
  ...
  it('should use lazy loading for all routes', () => {
    ...
    const lazyLoadingPercent = analysis.totalRoutes > 0
  ...
  it('should use dynamic import syntax for components', () => {
    ...
    const hasStaticComponentInRoute = /component\s*:\s*(?!.*=>.*import)[A-Z]\w+(?!.*import)/.test(content)
  ...
  it('property: all route paths should be valid URL paths', () => {
    ...
    const uniqueRoutes = analysis.routeDetails.filter((route, index, self) =>
