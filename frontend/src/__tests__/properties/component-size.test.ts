 ...
 * Get component info
 */
function getComponentInfo(filePath: string): { name: string; lines: number; sections: { template: number; script: number; style: number } } {
  const content = fs.readFileSync(filePath, 'utf-8')
...
describe('Property 3: Component Size Limit', () => {
  ...
  it('should not have any component exceeding 500 lines', () => {
    ...
    if (oversizedComponents.length > 0) {
      const errorMessage = oversizedComponents
        .map(c => `  - ${c.file}: ${c.lines} lines (exceeds ${MAX_COMPONENT_LINES})`)
        .join('\n')
      
      console.warn(`Warning: Found ${oversizedComponents.length} oversized component(s):\n${errorMessage}`)
    }
  ...
  it('should report component size statistics', () => {
    ...
    for (const stat of stats.slice(0, 10)) {
      ...
      console.log(`  - Template: ${stat.sections.template}, Script: ${stat.sections.script}, Style: ${stat.sections.style}`)
    }
  ...
  it('property: randomly selected components should be within reasonable size', () => {
    ...
    fc.assert(
      fc.property(
        fc.integer({ min: 0, max: vueFiles.length - 1 }),
        (index) => {
  ...
  it('should have components following single responsibility principle', () => {
    ...
    const avgComponentSize = componentFiles.reduce((sum, f) => sum + countLines(f), 0) / componentFiles.length
    const avgViewSize = viewFiles.reduce((sum, f) => sum + countLines(f), 0) / viewFiles.length
