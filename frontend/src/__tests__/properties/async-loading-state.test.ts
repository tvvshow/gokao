...
describe('Property 6: Async Operation Loading State', () => {
  ...
  it('should have loading states for components with async operations', () => {
    const componentsWithAsyncNoLoading: Array<{
      file: string;
      asyncCount: number;
    }> = [];
    ...
    console.log(
      `  - Components with async operations: ${totalAsyncComponents}`
    );
  ...
  it('should have skeleton or loading components available', () => {
    ...
    const hasSkeletonComponent = fs.existsSync(
      path.join(commonDir, 'SkeletonCard.vue')
    );

    // Check for error boundary component
    const hasErrorBoundary = fs.existsSync(
      path.join(commonDir, 'ErrorBoundary.vue')
    );

    console.log(`\nLoading/Error Components:`);
    console.log(
      `  - SkeletonCard component: ${hasSkeletonComponent ? 'Yes' : 'No'}`
    );
    console.log(
      `  - ErrorBoundary component: ${hasErrorBoundary ? 'Yes' : 'No'}`
    );
  ...
  it('should have proper try-catch error handling for async operations', () => {
    ...
    console.log(
      `  - Components with try-catch: ${componentsWithTryCatch}/${totalAsyncComponents}`
    );
    ...
    fc.assert(
      fc.property(
        ...
        (index) => {
          ...
          if (!analysis.hasLoadingState && !analysis.hasLoadingUI) {
            console.warn(
              `Component ${path.basename(file)} has async ops but no loading state`
            );
          }
  ...
  it('should have loading state in stores', () => {
    ...
    console.log(
      `  - Stores with loading state: ${storesWithLoading}/${storeFiles.length}`
    );
