 ...
 */
function countAnyUsages(filePath: string): {
  count: number;
  lines: number[];
  hasComments: boolean;
} {
 ...
 */
function getTypeCoverage(filePath: string): {
  hasTypes: boolean;
  typeImports: number;
  interfaces: number;
} {
...
describe('Property 2: TypeScript Type Safety', () => {
  ...
  it('should minimize any type usage across all files', () => {
    const filesWithExcessiveAny: Array<{
      file: string;
      count: number;
      lines: number[];
    }> = [];
    ...
    if (filesWithExcessiveAny.length > 0) {
      ...
      for (const f of filesWithExcessiveAny) {
        console.warn(
          `  - ${f.file}: ${f.count} usages (lines: ${f.lines.join(', ')})`
        );
      }
    }
  ...
  it('should have proper type definitions in types directory', () => {
    ...
    for (const file of typeFiles) {
      ...
      expect(
        ...
        `${file} should define interfaces or types`
      ).toBe(true);
  ...
  it('should have type coverage statistics', () => {
    ...
    console.log(
      `  - Files with type annotations: ${filesWithTypes}/${tsFiles.length} (${typeCoveragePercent.toFixed(1)}%)`
    );
  ...
  it('property: randomly selected files should have minimal any usage', () => {
    ...
    fc.assert(
      fc.property(fc.integer({ min: 0, max: tsFiles.length - 1 }), (index) => {
        ...
        if (result.count > MAX_ANY_PER_FILE) {
          console.warn(
            `File ${path.basename(file)} has ${result.count} 'any' usages`
          );
        }
